 ...
 */
const addTrigger = (triggerOptions) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield add(context, 'awscloudformation', 'Lambda', {
        trigger: true,
        modules: values,
        parentResource,
        functionName,
    ...
    if (values && values.length > 0) {
        for (let v = 0; v < values.length; v += 1) {
            yield copyFunctions(key, values[v], category, context, targetPath);
        }
    }
    const result = {};
 ...
 * @returns {null}
 */
const updateTrigger = (triggerOptions) => __awaiter(void 0, void 0, void 0, function* () {
    const { key, values, context, functionName, triggerEnvs = '[]', category, parentStack, targetPath, parentResource, triggerIndexPath, triggerPackagePath, triggerDir, triggerTemplate, triggerEventPath, skipEdit, } = triggerOptions;
    let update;
    try {
    ...
    try {
        yield update(context, 'awscloudformation', 'Lambda', {
            trigger: true,
            modules: values,
            parentResource,
            functionName,
        ...
        if (values && values.length > 0) {
            ...
            yield cleanFunctions(key, values, category, context, targetPath);
        }
...
const deleteDeselectedTriggers = (currentTriggers, previousTriggers, functionName, targetDir, context) => __awaiter(void 0, void 0, void 0, function* () {
    for (let p = 0; p < previousTriggers.length; p += 1) {
        if (!currentTriggers.includes(previousTriggers[p])) {
            ...
            yield context.amplify.deleteTrigger(context, `${previousTriggers[p]}`, targetPath);
        }
    }
});
const deleteTrigger = (context, name, dir) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        yield context.amplify.forceRemoveResource(context, 'function', name, dir);
    }
    catch (e) {
        throw new Error('Function plugin not installed in the CLI. You need to install it to use this feature.');
    }
});
const deleteAllTriggers = (triggers, functionName, dir, context) => __awaiter(void 0, void 0, void 0, function* () {
    const previousKeys = Object.keys(triggers);
    for (let y = 0; y < previousKeys.length; y += 1) {
        const targetPath = `${dir}/function/${functionName}`;
        yield context.amplify.deleteTrigger(context, functionName, targetPath);
    }
});
/**
 * @function triggerFlow
 ...
 */
const triggerFlow = (context, resource, category, previousTriggers = {}) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const wantTriggers = yield inquirer.prompt({
        name: 'confirmation',
        type: 'confirm',
    ...
    const askTriggers = yield learnMoreLoop('triggers', functionName, triggerMeta, triggerQuestion);
    ...
    for (let i = 0; i < askTriggers.triggers.length; i++) {
        ...
        const askTemplates = yield learnMoreLoop('templates', readableTrigger, templateMeta, templateQuestion);
 ...
 *  }"]
 */
const getTriggerPermissions = (context, triggers, category) => __awaiter(void 0, void 0, void 0, function* () {
    let permissions = [];
...
const learnMoreLoop = (key, map, metaData, question) => __awaiter(void 0, void 0, void 0, function* () {
    let selections = yield inquirer.prompt(question);
    while (
    ...
    Array.isArray(selections[key]) &&
        ...
        selections = yield inquirer.prompt(question);
    }
    return selections;
});
...
function openEditor(context, path, name) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (yield context.amplify.confirmPrompt.run(`Do you want to edit your ${name} function now?`)) {
            yield context.amplify.openEditor(context, filePath);
        }
    });
}
const copyFunctions = (key, value, category, context, targetPath) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (!dirContents.includes(`${value}.js`)) {
            ...
            yield openEditor(context, targetPath, value);
        }
    }
    catch (e) {
...
const cleanFunctions = (key, values, category, context, targetPath) => __awaiter(void 0, void 0, void 0, function* () {
    const pluginPath = context.amplify.getCategoryPlugins(context)[category];
    try {
...
const getTriggerEnvInputs = (context, path, triggerKey, triggerValues, currentEnvVars) => __awaiter(void 0, void 0, void 0, function* () {
            ...
            if (questions && questions.length) {
                for (let j = 0; j < questions.length; j += 1) {
                    if (!currentEnvVars ||
                        ...
                        const answer = yield inquirer.prompt(questions[j].question);
