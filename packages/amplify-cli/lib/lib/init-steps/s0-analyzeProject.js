...
function run(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const projectName = yield getProjectName(context);
        const envName = yield getEnvName(context);
...
function getEnvName(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const newEnvQuestion = () => __awaiter(this, void 0, void 0, function* () {
            const envNameQuestion = {
                type: 'input',
                name: 'envName',
                message: 'Enter a name for the environment',
                validate: input => !isEnvNameValid(input) ? 'Environment name should be between 2 and 10 characters (only lowercase alphabets).' : true,
            };
            ({ envName } = yield inquirer.prompt(envNameQuestion));
        });
        if (isNewProject(context)) {
            yield newEnvQuestion();
        }
        else {
            const allEnvs = context.amplify.getAllEnvs();
            if (allEnvs.length > 0) {
                if (yield context.amplify.confirmPrompt.run('Do you want to use an existing environment?')) {
                    const envQuestion = {
                        type: 'list',
                        name: 'envName',
                    ...
                    ({ envName } = yield inquirer.prompt(envQuestion));
                }
                else {
                    yield newEnvQuestion();
                }
