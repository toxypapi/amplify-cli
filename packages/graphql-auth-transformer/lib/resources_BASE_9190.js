...
class ResourceFactory {
    ...
    staticGroupAuthorizationExpression(rules, field) {
        ...
        for (const rule of rules) {
            ...
            if (rule.groupClaim) {
                if (customClaim) {
                    throw new graphql_transformer_core_1.InvalidDirectiveError(`@auth directive currently only supports one source for groupClaim.
  - Identified '${customClaim}' and '${rule.groupClaim}'`);
        ...
        return graphql_mapping_template_1.block('Static Group Authorization Checks', [
            graphql_mapping_template_1.comment(`Authorization rule: { allow: groups, groups: "${JSON.stringify(allowedGroups)}" }`),
            this.setUserGroups(customClaim),
            graphql_mapping_template_1.set(graphql_mapping_template_1.ref('allowedGroups'), graphql_mapping_template_1.list(allowedGroups.map(s => graphql_mapping_template_1.str(s)))),
            ...
            graphql_mapping_template_1.forEach(graphql_mapping_template_1.ref('userGroup'), graphql_mapping_template_1.ref('userGroups'), [
                graphql_mapping_template_1.forEach(graphql_mapping_template_1.ref('allowedGroup'), graphql_mapping_template_1.ref('allowedGroups'), [
                    graphql_mapping_template_1.iff(graphql_mapping_template_1.raw('$allowedGroup == $userGroup'), graphql_mapping_template_1.set(graphql_mapping_template_1.ref(staticGroupAuthorizedVariable), graphql_mapping_template_1.raw('true')))
                ])
            ])
        ]);
    }
...
groupsField: "${rule.groupsField || constants_1.DEFAULT_GROUPS_FIELD}" }`);
...
identityClaim: "${rule.identityField || rule.identityClaim || constants_1.DEFAULT_IDENTITY_FIELD}" }`)
    ...
    dynamicGroupAuthorizationExpressionForUpdateOrDeleteOperations(rules, fieldBeingProtected, variableToCheck = 'ctx.args.input', variableToSet = graphql_transformer_common_1.ResourceConstants.SNIPPETS.IsDynamicGroupAuthorizedVariable) {
        ...
        for (const rule of rules) {
            ...
            groupAuthorizationExpressions = groupAuthorizationExpressions.concat(graphql_mapping_template_1.comment(`Authorization rule${fieldMention}: { allow: ${rule.allow}, groupsField: "${groupsAttribute}" }`), 
    ...
    dynamicGroupAuthorizationExpressionForReadOperations(rules, variableToCheck = 'ctx.result', variableToSet = graphql_transformer_common_1.ResourceConstants.SNIPPETS.IsDynamicGroupAuthorizedVariable, defaultValue = graphql_mapping_template_1.raw(`$util.defaultIfNull($${variableToSet}, false)`)) {
        ...
        for (const rule of rules) {
            ...
            const groupsAttribute = rule.groupsField || constants_1.DEFAULT_GROUPS_FIELD;
            groupAuthorizationExpressions = groupAuthorizationExpressions.concat(graphql_mapping_template_1.comment(`Authorization rule: { allow: ${rule.allow}, groupsField: "${groupsAttribute}" }`), graphql_mapping_template_1.set(graphql_mapping_template_1.ref('allowedGroups'), graphql_mapping_template_1.ref(`util.defaultIfNull($${variableToCheck}.${groupsAttribute}, [])`)), graphql_mapping_template_1.forEach(graphql_mapping_template_1.ref('userGroup'), graphql_mapping_template_1.ref('userGroups'), [
    ...
    setUserGroups(customGroup) {
        if (customGroup) {
            return graphql_mapping_template_1.block(`Using groupClaim: ${customGroup} as source for userGroup`, [
                ...
                graphql_mapping_template_1.iff(graphql_mapping_template_1.raw('$util.isString($userGroup)'), graphql_mapping_template_1.set(graphql_mapping_template_1.ref('userGroup'), graphql_mapping_template_1.raw('[$userGroup]'))),
            ]);
        }
...
//# sourceMappingURL=resources_BASE_9190.js.map